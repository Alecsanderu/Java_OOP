package com.codecool.crudpractice.user.security;
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
            ...
            .and()
                ...
                .antMatchers("/api/v1/register").permitAll() // allowed by anyone
                .antMatchers(HttpMethod.GET, "/api/v1/employees").authenticated() // allowed only when signed in
                .antMatchers(HttpMethod.GET, "/api/v1/employees/**").authenticated() // allowed only when signed in
                .antMatchers(HttpMethod.POST, "/api/v1/employees/**").hasRole("ADMIN") // allowed only when signed in
                .antMatchers(HttpMethod.DELETE, "/api/v1/employees/**").hasRole("ADMIN") // allowed only when signed in
